...
class Robot(object):
    def __init__(self, addr=0x60, left_id=1, right_id=2, left_trim=0, right_trim=0, cutter_id=3,
                 stop_at_exit=True):
        ...
        self._cutter = self._mh.getMotor(cutter_id)
        self._left_trim = left_trim
        self._right_trim = right_trim
        # Start with motors turned off.
        self._left.run(Adafruit_MotorHAT.RELEASE)
        self._right.run(Adafruit_MotorHAT.RELEASE)
        self._cutter.run(Adafruit_MotorHAT.RELEASE)
        # Configure all motors to stop at program exit if desired.
        if stop_at_exit:
            atexit.register(self.stopall)
    ...
    def _cutter_speed(self, speed):
        """Set the speed of the cutter disc
        """
        assert 0 <= speed <= 255, 'Speed must be a value between 0 to 255 inclusive!'
        self._cutter.setSpeed(speed)
    ...
    def cutterforward(self, speed, seconds=None):
        """Spin the cutter disc at specified speed. Will start spinning and
        return unless a seconds value is specified.
        """
        self._cutter_speed(speed)
        self._cutter.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
            time.sleep(seconds)
            self.stop()

    def cutterbackward(self, speed, seconds=None):
        """Spin the cutter disc at specified speed. Will start spinning and
        return unless a seconds value is specified.
        """
        self._cutter_speed(speed)
        self._cutter.run(Adafruit_MotorHAT.BACKWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
            time.sleep(seconds)
            self.stop()
