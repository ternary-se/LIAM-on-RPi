...
class Sensorread(object):
    ...
    def readcoils(self):
        # read # of hits of right BWF coil during 0.1s
        rhitcount = 0
        lhitcount = 0
        prev_rinput = 0
        prev_linput = 0
        ...
        while (elapsed_bwf <= .10):
          #take a reading
          rinput = GPIO.input(24)
          linput = GPIO.input(25)
...
### escapes: make shure the robot really stops before changing direction to avoid great currents
    ...
    def escapehitrightcoil(self):
        ...
        while (a == 0):
          ...
          if ( 17 <= coil[0] <= 27) or (timediff >= turntime) or (wheelload > 400):
            robot.stop()
            time.sleep(0.85)
            a = 1
    ...
    def escapehitleftcoil(self):
        ...
        while (a == 0):
          ...
          if ( 17 <= coil[1] <= 27) or (timediff >= turntime) or (wheelload > 400):
            robot.stop()
            time.sleep(0.85)
            a = 1
            if (timediff >= turntime):
